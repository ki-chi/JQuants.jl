...
@testset "Listed issues information" begin
    code = "86970"  # JPX
    date = today() - Month(3)  # 3 months ago (because the available periods for free plan is limited)
    listed_info = fetch(ListedInfo(code=code, date=date))
...
@testset "Daily prices" begin
    date = today() - Month(3)
    daily_quotes = fetch(PricesDailyQuotes(date=date))
    expected_colnames = [
    ...
    daily_quotes_null = fetch(PricesDailyQuotes(date="2023-01-01"))
